...
### handy functions ###
def train_stochastic_hessian_model(
    y, tx, y_test, tx_test, initial_weights, max_iters, gamma, batch_size
):
    test_loss_list = []
    ### train the model ###
    w = initial_weights
...
if __name__ == "__main__":
    ...
    for i in range(nb_of_samples):
        print("Iteration: ", i + 1)
        average_test_loss_list += np.array(
            train_stochastic_hessian_model(
                y, tx, y_test, tx_test, initial_weights, max_iters, gamma, batch_size
            )
        )
    average_test_loss_list /= nb_of_samples
    print(
        "AVERAGE TEST LOSS LIST HESSIAN BATCH SIZE ",
        batch_size,
        ": ",
