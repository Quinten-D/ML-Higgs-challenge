...
def lambda_grid_search(y_train, x_train, y_test, x_test, lambda_grid, max_iters, gamma):
    ...
    for lambda_ in lambda_grid:
        weight, _ = reg_logistic_regression(
            y=y_train,
            tx=x_train,
            lambda_=lambda_,
            initial_w=x_train[0],
    ...
    optimal_weight, optimal_lambda = (
        weight_list[np.argmin(loss_list)],
        lambda_grid[np.argmin(loss_list)],
    )
...
def cross_validation_tuning(y, x, k_fold, lambda_grid, max_iters, gamma):
    ...
    for k in range(k_fold):
        ...
        optimal_weight, optimal_lambda = lambda_grid_search(
            y_train, x_train, y_test, x_test, lambda_grid, max_iters, gamma
        )
    ...
    final_weight, final_lambda = weight_lambda_pairs[np.argmin(loss_list)]
    final_loss = np.min(loss_list)
    # return (weight, lambda) pair with the lowest loss
    return final_weight, final_loss, final_lambda
